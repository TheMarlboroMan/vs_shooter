...
void  Controlador_controles::preloop(DFramework::Input& input, float delta)
{
	...
	{	
		//TODO: Muy mejorable.
		controles[Input::j1_registrar]={1, tinput::registrar, Input::j1_registrar, "P1: Start", input.localizar_entrada(Input::j1_registrar)};
		...
		controles[Input::j2_registrar]={2, tinput::registrar, Input::j2_registrar, "P2: Start", input.localizar_entrada(Input::j2_registrar)};
		...
		controles[Input::j3_registrar]={3, tinput::registrar, Input::j3_registrar, "P3: Start", input.localizar_entrada(Input::j3_registrar)};
...
void Controlador_controles::finalizar_configuracion()
{
	...
	for(const auto& par : controles)
	{
		...
		switch(c.tipo)
		{
			case tinput::registrar: 	config.mut_registrar(c.jugador, val); break;
