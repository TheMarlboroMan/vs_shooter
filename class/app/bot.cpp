...
void Bot::turno(const Mapa& mapa, std::vector<Disparador>& vd, float delta)
{
	...
	t_disparo+=delta;
	const auto& v=destino->acc_poligono().get_rotation_center();
	const auto& c=poligono.get_rotation_center();
...
void Bot::intento_disparo(const Mapa& mapa, std::vector<Disparador>& vd)
{
	...
	ldt::vector_2d<double> vect=vector_from_points(c, v);
	auto ang=vect.get_angle_deg();
...
void Bot::trazar_ruta(const Mapa& mapa)
{
	...
	auto pt_fin=destino->acc_poligono().get_rotation_center();


//	const Nodo_ruta * ini=mapa.localizar_nodo_cercano(pt_ini);
	Punto_ruta pr{poligono.get_rotation_center(), false};
	...
	if(res.resuelto)
	{
		...
		{
			if(!ldt::point_in_polygon(poligono, pt))
			{
...
void Bot::calcular_angulo_destino(ldt::point_2d<double> dest)
{
	...
	auto pt=poligono.get_rotation_center();
	auto angulo_destino=vector_from_points(pt, dest).get_angle_deg();

	if(std::isnan(angulo_destino))
	{
...
//LOG<<"PARA IR DE "<<angulo<<" A "<<angulo_destino<<" "<<direccion_giro<<" PARA UN TOTAL DE "<<grados_a_girar<<std::endl;
	...
	distancia_anterior=pt.distance_to(dest);
}
...
//LOG<<"TRAZANDO POSICION ATAQUE..."<<std::endl;
	...
	double dist=c.distance_to(c);

	if(dist > 250.0)
	{
		ldt::vector_2d<double> vect=vector_from_points(pt_fin, c);
...
void Bot::dibujar(Representador& r, ldv::screen& pantalla, const ldv::camera& camara, const ldv::resource_manager&) const
{
