...
class Exportador
{
	...
	std::string serializar(const std::vector<Obstaculo>&, const std::vector<DLibH::Punto_2d<double>>&, const std::vector<Generador_items>&);

	private:

	Herramientas_proyecto::Dnot_token 		serializar_obstaculo(const Obstaculo&);
	Herramientas_proyecto::Dnot_token 		serializar_punto_inicio(DLibH::Punto_2d<double>);
